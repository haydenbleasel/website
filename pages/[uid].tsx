  ...
  <Layout backHref="/" backLabel="Home">
    <div className="grid gap-8">
      <div className="grid gap-1">
        ...
        {data.description && (
          <p className="text-md text-gray-900 font-normal">{data.description}</p>
        )}
        <p className="text-sm text-gray-500">Last updated at {format(parseISO(last_publication_date), 'MMM dd, yyyy')} </p>
      </div>
      {data.cover.url && (
        <div className="rounded-sm flex overflow-hidden">
          <Image src={data.cover.url} alt={data.cover.alt ?? ""} width={480} height={480 * (data.cover.dimensions.height / data.cover.dimensions.width)} priority />
        </div>
      )}
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  const { data, last_publication_date } = await getPage(params?.uid as string, 'landing-page') as PrismicDocumentWithUID;
